    ...
    class Menu
      ...
      def item id, content='', options={}, html_options={}, &block        
        if block_given?
          ...
          if (@permits.include?(id) && !id.blank?) || @template.request.path.start_with?("/admin") || @template.request.path.start_with?("/report/cmcc_reports/show") || @template.request.path.start_with?("/personal")
            @items << id
            html_options[:id] = id
