...
function WebFXLoadTree(sText, sXmlSrc, sAction, selectType, sGroupName, appDatas, sBehavior, sIcon, sOpenIcon) {
    //alert("2WebFXLoadTree(");
	// call super
	this.WebFXTree = WebFXTree;
	this.WebFXTree(sText, sAction, selectType, sGroupName, appDatas, sBehavior, sIcon, sOpenIcon);
...
function WebFXLoadTreeItem(sText, sXmlSrc, sAction, selectType, sGroupName, appDatas, eParent, sIcon, sOpenIcon) {
	// call super
	this.WebFXTreeItem = WebFXTreeItem;
	this.WebFXTreeItem(sText, sAction, selectType, sGroupName, appDatas, eParent, sIcon, sOpenIcon);
...
function _xmlTreeToJsTree(oNode) {
	...
	var selectType = 0;
	var sSelectType = oNode.getAttribute("selectType");
	if(sSelectType!=null&&sSelectType!=""){
	    try{
	        selectType = parseInt(sSelectType);
	    }catch(e){
	        selectType = 0;
	    }
	}
	var sGroupName = oNode.getAttribute("groupName");
	var appDatas = new Array();
	var curAppDataNames = webFXTreeConfig.appDataNames[sGroupName];
	if(curAppDataNames){
	    for(var i=0;i<curAppDataNames.length;i++){
            appDatas[curAppDataNames[i]] = oNode.getAttribute(curAppDataNames[i]);
        }
	}
	...
	var jsNode;
	if (src != null && src != "")
		jsNode = new WebFXLoadTreeItem(text, src, action,selectType, sGroupName, appDatas, parent, icon, openIcon);
	else
		jsNode = new WebFXTreeItem(text, action,selectType, sGroupName, appDatas, parent, icon, openIcon);
