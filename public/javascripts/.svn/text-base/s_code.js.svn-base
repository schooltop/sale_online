...
//The s_account variable is being generated 
var s_account="ritzbulgaricom" //change to ritzbulgaricom when moved to production
...
s.linkInternalFilters="javascript:,ritzcarlton.com,bulgarihotels.com"
...
s.siteID=s_siteID;     // leftmost value in pagename :JDG:20080813:This is being set dynamically server-side
...
/* You may insert any plugins you wish to use here.                 */
		...
		if(!s.campaign){
			s.campaign=s.getQueryParam('utm_campaign,EP_MID');}
			s.campaign=s.getValOnce(s.campaign,"cname",0);
		}

		/* Set Epsilon Recipient ID */
		s.eVar29=s.getQueryParam('EP_RID');
	
		/*Set Marriott Referral ID 30-Day Latency  &  locale*/
		s.eVar38=s.getQueryParam('mtid');
		s.eVar38=s.getValOnce(s.eVar38,"mtid1",0);

		s.eVar39=s.getQueryParam('locale');
		s.eVar39=s.getValOnce(s.eVar39,"cname",0);

		//Marriott Refferal ID Same-Session Latency
		s.eVar43=s.getQueryParam('mtid');
		s.eVar43=s.getValOnce(s.eVar43,"mtid2",0);

		//Marriott Refferal ID Same-Day Latency
		s.eVar44=s.getQueryParam('mtid');
		s.eVar44=s.getValOnce(s.eVar44,"mtid3",0);

		//Marriott Refferal ID 7-Day Latency
		s.eVar45=s.getQueryParam('mtid');
		s.eVar45=s.getValOnce(s.eVar45,"mtid4",0);
				
		/* For Cross Visit Participation */
		s.eVar32=s.crossVisitParticipation(s.campaign,'s_cpm','30','5','>','purchase');

        var dtToday = new Date();
        var thisYear = '' + dtToday.getFullYear();        
			
		var temphr=s.getTimeParting('h','-5',thisYear); // Set hour 
		var tempday=s.getTimeParting('d','-5',thisYear); // Set day
		var tempweek=s.getTimeParting('w','-5',thisYear); // Set Weekend / Weekday
...
s.visitorNamespace="ritzcarltonhotelcompany"
s.trackingServer="data.ritzcarlton.com"
s.trackingServerSecure="sdata.ritzcarlton.com"
