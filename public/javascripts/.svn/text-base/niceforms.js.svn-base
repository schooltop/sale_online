...
var textareaSidePadding = 0; /* gab controllare  = 10*/
var fieldErrorClass = "FieldKO";
...
function niceform(nf) {
	...
	nf.start = function () {
	    ...
	    for (var w = 0; w < allSelects.length; w++) {
	        if (allSelects[w].className != 'noniceform') {
...
function inputText(el) { //extent Text inputs
	...
	el.dummy.className = "NFTextCenter"  + ((el.oldClassName.indexOf(fieldErrorClass) != -1)?(" " + fieldErrorClass):"");
...
function inputCheck(el) { //extend Checkboxes
	...
	el.dummy.ref = el;
	//if(isIE == false) {el.dummy.style.left = findPosX(el) + 'px'; el.dummy.style.top = findPosY(el) + 'px';	}
	//else {el.dummy.style.left = findPosX(el) + -2+ 'px'; el.dummy.style.top = findPosY(el) + 11 + 'px';}
	el.dummy.onclick = function() {
		if(!this.ref.checked) {
			this.ref.checked = true;
...
function textarea(el) { //extend Textareas
	...
	el.right.className = "NFTextareaRight"  + ((el.oldClassName.indexOf(fieldErrorClass) != -1)?(" " + fieldErrorClass):"");
...
function selects(el) { //extend Selects
	...
	el.dummy.className = "NFSelect"  + ((el.oldClassName.indexOf(fieldErrorClass) != -1)?(" " + fieldErrorClass):"");
	el.dummy.style.width =  180 + 'px';
	el.dummy.ref = el;
	el.left = document.createElement('img');
	el.left.src = imagesPath + "0.png";
	...
	el.dummy.onclick = function () {
	    ...
	    if (this.ref.opt.offsetHeight > selectMaxHeight) {
	        ...
	        this.ref.opt.style.width = this.ref.offsetWidth - selectRightWidthSimple + 18 + 'px';
	    }
	    //AlessandroG: Fix?
	    this.ref.opt.style.width = this.offsetWidth + 'px';
	    //alert("refoffsetwidth:" + this.offsetWidth + " - width:" + this.ref.opt.style.width + " - rightwidthsimple:" + selectRightWidthSimple + " - MaxHeight: " + selectMaxHeight + " - wightwidthscroll:" + selectRightWidthScroll);
	}
