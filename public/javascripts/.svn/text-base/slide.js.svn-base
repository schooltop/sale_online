...
var slideShow = function () {
    var bxs, bxe, fxs, fxe, ys, ye, ta, ia, ie, st, ss, ft, fs, xp, yp, ci, t, tarl;
    ...
    return {
        init: function () {
            ...
            for (i = 0; i < len; i++) {
                var id = t[i].value;
                t[i].onclick = new Function("slideShow.getimg('" + tar[0] + "')");
                if (i == 0) { this.getimg(tar[0]) }
            }
            tarl = tar.length;
        },
        scrl: function (d) {
        ...
        getimg: function (id) {
            if (f6Moving == false) {//F6 Gallery FIX
                f6Moving = true; //F6 Gallery FIX
                ...
                if (!document.getElementById(id)) {
                    var i = document.getElementById('0');
                    i = i.cloneNode(true);
...
//            }
        ...
        fdin: function (i) {
            ...
            if (i.av >= 100) { if (auto) { this.auto() }; clearInterval(i.timer); ci = i; f6Moving = false; } //F6 Gallery FIX
        },
        fdout: function (i) {
